export const DASHBOARD_OVERVIEW_SECTIONS_LOGIC = String.raw`
      function formatBytes(value) {
        ...
        if (numeric < 1024 * 1024) return formatMetricNumber(numeric / 1024, 1) + ' KB';
        return formatMetricNumber(numeric / 1024 / 1024, 1) + ' MB';
      }

      function marketDataPrimaryIndex(status) {
        const indexes = status && Array.isArray(status.indexes) ? status.indexes : [];
        return indexes.find(function (item) { return item.chain === 'ethereum'; }) || indexes[0] || null;
      }

      function renderMarketDataIndexStatus() {
        const status = state.data.marketDataIndexStatus;
        const index = marketDataPrimaryIndex(status);
        const isDatabaseIndex = status && status.provider === 'mysql-market-events';
        const labelTitle = isDatabaseIndex
          ? (state.language === 'zh' ? '市场索引' : 'Market Index')
          : (state.language === 'zh' ? '本地索引' : 'Local Index');
        const labelBlock = state.language === 'zh' ? '最新区块' : 'Latest Block';
        const labelSize = isDatabaseIndex
          ? (state.language === 'zh' ? '记录数' : 'Rows')
          : (state.language === 'zh' ? '文件大小' : 'File Size');
        const labelState = state.language === 'zh' ? '状态' : 'Status';
        const stateText = status && status.indexing
          ? (state.language === 'zh' ? '补区块中' : 'Indexing')
          : (index && index.exists ? (state.language === 'zh' ? '已就绪' : 'Ready') : (state.language === 'zh' ? '待生成' : 'Pending'));
        const sourceLabels = index && Array.isArray(index.sources)
          ? index.sources.map(function (source) { return source.label; }).filter(Boolean)
          : [];
        const sourceText = sourceLabels.length ? sourceLabels.join(' / ') : '--';
        qsa('[data-market-index-panel]').forEach(function (panel) {
          const title = panel.querySelector('[data-market-index-title]');
          const sources = panel.querySelector('[data-market-index-sources]');
          const block = panel.querySelector('[data-market-index-block]');
          const size = panel.querySelector('[data-market-index-size]');
          const stateNode = panel.querySelector('[data-market-index-state]');
          const blockLabel = panel.querySelector('[data-market-index-block-label]');
          const sizeLabel = panel.querySelector('[data-market-index-size-label]');
          const stateLabel = panel.querySelector('[data-market-index-state-label]');
          if (title) title.textContent = labelTitle;
          if (sources) sources.textContent = sourceText;
          if (blockLabel) blockLabel.textContent = labelBlock;
          if (sizeLabel) sizeLabel.textContent = labelSize;
          if (stateLabel) stateLabel.textContent = labelState;
          if (block) block.textContent = index && index.toBlock ? String(index.toBlock) : '--';
          if (size) {
            size.textContent = index
              ? (isDatabaseIndex
                ? formatInteger((Number(index.flashloanRows) || 0) + (Number(index.liquidationRows) || 0))
                : formatBytes(index.fileSizeBytes))
              : '--';
          }
          if (stateNode) {
            stateNode.textContent = stateText;
            stateNode.classList.toggle('status-blue', Boolean(status && status.indexing));
            stateNode.classList.toggle('status-good', Boolean(index && index.exists && !(status && status.indexing)));
            stateNode.classList.toggle('status-warn', !(index && index.exists) && !(status && status.indexing));
          }
        });
      }
