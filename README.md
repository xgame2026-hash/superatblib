...
版本 1.6.9 以新的协议和数据结构为准。旧版本数据和旧客户端行为不作为兼容目标，所有客户端应升级到当前版本后再运行。
...
启动排队后，LIQ2 每 5 分钟向 `https://privateapi.superarb.ai/heartbeat` 上报一次连接状态。短时心跳失败不会撤销排队资格；连续 6 小时没有任何成功心跳时，privateapi 才会强制下线。主动暂停、在系统设置中退出、套餐/RPC 服务到期会立即下线。以上状态都会从 `https://privateapi.superarb.ai/online-users` 排行榜移除，用户及钱包历史记录不会删除。
...
编辑 `.env.wallet2`，把 `PRIVATE_KEY` 换成第二钱包的私钥，并把 `DASHBOARD_PORT` 改成 `4312`。授权码、BNB RPC 和 app token 可以与第一个钱包相同。

分别在两个终端启动：

```bash
npm run dashboard
npm run dashboard:profile -- wallet2
```

第二个命令会先检查 `.env.wallet2` 是否存在有效私钥，但不会输出私钥。服务端以完整钱包地址隔离队列与计费，因此两个不同私钥会分别扣费。
...
默认 `UPDATE_MODE=automatic`。发现远程 `main` 有新提交后，本地控制台会执行 fast-forward 拉取、`npm ci`、生产构建，并在构建成功后自动停止旧实例和重启。

当多个钱包 profile 同时运行时，升级器会记录所有活动实例的 PID、profile 和端口；升级完成后统一停止旧实例，再按原 profile 和端口恢复。工作区有未提交文件或无法 fast-forward 时会停止升级，不会覆盖本地修改。
...
面板每五分钟检查一次主线版本。发现新版本时，会先播放“发现新版本”提示音。设置里的“更新”默认是“自动”：浏览器确认提示音已经开始播放后，本地服务会自动拉取主线、安装锁定依赖、验证生产构建、重启面板并刷新页面；新版本健康检查通过后，再播放“升级完成”提示音。选择“手动”时只提示发现新版本，并要求用户在控制器运行 `npm run update`。全局声音关闭时不会等待提示音；自动模式仍会继续升级。

控制器只允许在 `main` 分支和干净工作区执行，并且只接受 fast-forward 更新。存在本地改动、安装失败或构建失败时会停止升级，不会写入成功凭证，也不会自动重启。自动升级状态保存在 `.superarb/auto-update-status.json`。
...
手动运行时，升级成功后会提示重新启动；由面板自动触发时会自动完成重启。安装或构建失败不会写入成功凭证，再次运行 `npm run update` 会继续未完成的升级。
